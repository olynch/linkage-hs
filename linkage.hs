...
makeOverlayScheme l ps = (OverlayScheme ps pm, x)
  where
    ...
    x = V.fromList $ do
      ...
      [p1,p2]
    pm = V.fromList $ map (\i -> findIndex (==i) idxs) [0..n-1]
...
adjustLinkage n p l ps = buildPositions os $ fmap realToFrac $ opt $ init
  where
...
main :: IO ()
main = play (InWindow "Linkage" (400,400) (0,0)) white 5 initWorld drawWorld updateWorld stepWorld
